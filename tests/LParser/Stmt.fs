...
module Parser =
  ...
  let assignP = (sp word << sp (symbol &":=")) >>= fun x -> map (fun e -> Assign (x, e)) (Expr.Parser.parse ())
  ...
  let whileP p = (symbol &"while" |> sp) >>
                 (Expr.Parser.parse () |> sp) >>= fun e -> (p() |> cparen' |> map (fun op -> While (e, op)))

  let term p = readP <|> writeP <|> assignP <|> ifP p <|> whileP p

  let seqP p = (term p << sp (char ';')) >>= fun x -> (p() |> sp |> map (fun y -> Seq (x, y)))
