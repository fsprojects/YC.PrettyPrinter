...
let rec exprPrinter (e : Expr.t) : string =
    ...
    | Num x -> x.ToString () + "\n"
    | Var x -> x + "\n"
    | BinOp (op, x, y) -> String.Format("{0}\n{1}{2}", op.ToString (), exprPrinter x, exprPrinter y)
...
let rec printer (s : Stmt.t) =
    ...
    | If (e, s1, s2) -> String.Format("if\n{0}{1}{2}", exprPrinter e, printer s1, printer s2)
    | While (e, s) -> String.Format("while\n{0}{1}", exprPrinter e, printer s)


let f x = 
    if x 
    then failwith "error in true branch"
