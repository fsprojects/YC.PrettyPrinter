...
module Parser =
    ...
    let rec pLeftAssoc' opParser pNext =
        opParser >>= fun op -> pNext >>= fun r -> mreturn (fun l -> BinOp (op, l, r))
    let rec pLeftAssoc opParser pNext =
        let app f x = f x 
        pNext >>= fun l -> fold app l (pLeftAssoc' opParser pNext)
    let rec pRightAssoc opParser pNext =
        pNext >>= fun leftArg -> (opParser >>= fun op -> (pRightAssoc opParser pNext) >>= fun s -> mreturn (BinOp (op, leftArg, s))) <|> mreturn leftArg
