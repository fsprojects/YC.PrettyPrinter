module YC.PrettyPrinter.StructuredFormat
...
let (++) (doc1 : Doc) doc2 = (doc1 >||< doc2) >//< (doc1 >-< doc2)

/// Join, possible break with indent = 1.
let (--)  doc1 doc2 = (doc1 >||< doc2) >//< (doc1 >-< Indent(1, doc2))
...
let rec private tagListL tagger = 
    ...
    | [] -> emptyL
    | [x] -> x
    | x::xs -> (tagger x) >||< tagListL tagger xs
...
let private boundedUnfoldL (itemL : 'a -> Doc) (project : 'z -> ('a * 'z) option) (stopShort : 'z -> bool) (z : 'z) maxLength =
    let rec consume n z =
        ...
        then [wordL "..."] 
            ...
            | None -> []  (* exhaused input *)
            | Some (x, z) -> if n <= 0 
                             then [wordL "..."]                 (* hit print_length limit *)
