...
let rec docToFormats wid doc = 
    match doc with
    | Text(str) -> 
        if str.Length <= wid then Map.empty.Add(new Frame(str.Length, str.Length, str.Length), stringToFormat str)
        else Map.empty
    | Indent(ind, indDoc) -> 
    ...
    | Fill (doc1, shift, doc2) ->
        ...
        let dlist2 = docToFormats wid doc2
        let clearlist2 l = Map.filter (fun key (r : Format) -> not (tooWide wid l r)) dlist2
        let fillForL (l : Format) = Map.fold (fun acc key r -> update (Format.addFill(l,r,shift)) acc) Map.empty dlist2
